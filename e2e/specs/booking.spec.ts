...
test("staff books a delivery from the New Order form and gets a tracking link", async ({ page, request }) => {
  ...
  await page.getByLabel("Customer name").fill("Tanya Booker");
  await page.getByLabel("Phone").fill(freshPhone());
  await page.getByLabel("Pickup", { exact: true }).fill("10A Duppy Ave, Kingston");
  await page.getByLabel("Destination", { exact: true }).fill("5 Rose Ave, Old Rose Garden");
  await page.getByLabel("Landmark", { exact: true }).fill("behind the red gate");
  ...
  await page.getByLabel("Order value").fill("4500");
  await page.getByLabel("Delivery fee", { exact: true }).fill("350");
  ...
  await page.getByLabel("Priority", { exact: true }).selectOption({ label: "Express" });
  await page.getByLabel("Delivery instructions").fill("Gate code 4421, ring twice");
  ...
  // the job lands in the dispatch queue
