...
export async function trackingRoutes(app: FastifyInstance, ctx: AppCtx): Promise<void> {
  ...
  app.post<{ Params: { token: string } }>("/api/tracking/:token/rate", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req) => {
    ...
    const rating = await ctx.prisma.rating.create({
      data: { jobId: job.id, riderId: job.riderId, merchantId: null, raterType: "customer", score: body.score, comment: body.comment || null },
    });
  ...
  app.post<{ Params: { token: string } }>("/api/tracking/:token/rate-merchant", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req) => {
    ...
    const existing = await ctx.prisma.rating.findUnique({ where: { jobId_raterType: { jobId: job.id, raterType: "customer_merchant" } } });
    if (existing) throw httpErrors.createError(409, "You've already rated this merchant.");
    const rating = await ctx.prisma.rating.create({
      data: { jobId: job.id, riderId: null, merchantId: job.merchantId, raterType: "customer_merchant", score: body.score, comment: body.comment || null },
    });
    return { ok: true, rating: { id: rating.id, score: rating.score } };
  });
}
