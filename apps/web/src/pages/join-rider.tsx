...
export function JoinRider(): React.JSX.Element {
  ...
  const canSubmit = name.trim().length > 0 && phone.replace(/[^\d]/g, "").length >= 7 && password.length >= 8;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await apiFetch<{ status: "pending" | "active" }>(API.riders.signup, {
        method: "POST",
        body: JSON.stringify({ name, phone, vehicle, plate: plate || undefined, password }),
      });
      setResult(res);
    } catch (err) {
        ...
        <div className="card w-full max-w-sm space-y-2 text-center">
          ...
          <p className="text-sm text-zinc-300">
            {result.status === "active"
              ? "You're already approved and ready to go — sign in with your phone number and password."
              : "A dispatcher will review your application shortly. You'll be able to sign in with your phone number and password once approved."}
          </p>
    ...
    <div className="flex min-h-dvh items-center justify-center bg-zinc-950 p-4">
      <form onSubmit={(e) => void submit(e)} className="card w-full max-w-sm space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">At least 8 characters. You'll use your phone number + this password to sign in once approved.</p>
        </div>
